 ...
 */
function resolveLookbackDays(period, defaultPeriod = 'month') {
  if (typeof period === 'string' && Object.prototype.hasOwnProperty.call(PERIOD_TO_DAYS, period)) {
    return { period, lookbackDays: PERIOD_TO_DAYS[period] };
  }
  return { period: defaultPeriod, lookbackDays: PERIOD_TO_DAYS[defaultPeriod] };
}
 ...
 */
export async function recommendModel(req, res) {
  try {
    ...
    const recommendation = await modelRecommendationService.recommendModel(
      taskDescription,
      constraints
    );
 ...
 */
export async function getModelAnalysis(req, res) {
  try {
    ...
    for (const model of analysis) {
      ...
      // lookbackDays is a trusted integer from the whitelist above; build the
      // modifier string and pass it as a bound parameter to datetime().
      ...
      for (const error of errors) {
        if (error.response_metadata) {
          try {
            const metadata = JSON.parse(error.response_metadata);
 ...
 */
export async function getOptimizationOpportunities(req, res) {
  try {
    // Validate period via whitelist to prevent SQL injection
    ...
    for (const record of records) {
      ...
      const complexity = modelRecommendationService.analyzeTaskComplexity(
        record.task_description || 'unknown'
      );
    ...
    const opportunityList = Object.values(opportunities).map(opp => {
