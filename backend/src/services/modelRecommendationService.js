 ...
 */
export async function calculateSafetyScore(model) {
    try {
        // Get model analysis (cached stats)
        const analysis = (await db.getQuery('SELECT success_rate, error_count FROM model_analysis WHERE model = ?', [model]));
        if (!analysis) {
 ...
 */
export async function calculateCostBenefit(model, complexity, costWeightage = 0.3, activeModels) {
    try {
        ...
        const maxCost = Math.max(...models.map(m => (m.input_price ?? 0) + (m.output_price ?? 0) / 2));
    ...
    try {
        ...
        return {
            ...
            alternatives: alternatives.map(alt => ({
                ...
                savings: alt.model.includes('Haiku') && recommendedModel.includes('Opus')
                    ? '75-85%'
...
export async function refreshModelAnalytics() {
    try {
        ...
        const models = (await db.allQuery('SELECT DISTINCT model FROM usage_records WHERE timestamp >= datetime(\'now\', \'-30 days\')'));
      ...
      `, [model]));
            if (stats && (stats.total_requests || 0) > 0) {
                ...
                console.log(`Updated analytics for ${model}: ${stats.total_requests} requests, ${((stats.success_rate || 0) * 100).toFixed(1)}% success rate`);
            }
        }
...
//# sourceMappingURL=modelRecommendationService.js.map