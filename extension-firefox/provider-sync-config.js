...
(function exposeProviderSyncConfig(root) {
  ...
  function getConfiguredProviderKeys(providers) { return new Set((Array.isArray(providers) ? providers : []).filter((provider) => typeof provider?.key === 'string' && typeof provider?.plan_name === 'string' && provider.plan_name.trim() !== '').map((provider) => provider.key)); }
